...
description: Особенность работы с CustomNetTables в Dota 2 Workshop Tools.
template: doc
meta:
...
Все **названия таблиц** нужно **обязательно** декларировать в специальном KV файле `your_addon\scripts\custom_net_tables.txt`.
Без этого вы не сможете записывать и читать данные из **CustomNetTables**.
...
<Aside type="danger" title="Ограничение по весу данных">
    ...
    ![Ошибка](@assets/images/fatal_erros/custom_net_tables_userdata_size_limits.png)
</Aside>

## Совет

Старайтесь разделять данные по разным **таблицам** и **ключам**, чтобы снизить нагрузку на Net трафик.
Каждый пакет передается в полной мере, а не только измененные данные в передаваемой таблице.